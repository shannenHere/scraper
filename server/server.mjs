...
const buildUrl = (req, subpath) =>
  `${req.protocol}://${req.get('host')}${path.join(req.baseUrl, subpath)}`;
...
router.get('/apps/search', function (req, res, next) {
  ...
  const opts = { term: req.query.q, ...req.query };
...
/* Search suggest */
router.get('/apps/suggest', function (req, res, next) {
  if (!req.query.suggest) {
    return res.status(400).json({ message: 'Query parameter suggest is required' });
  }

  const toJSON = (term) => ({
    term,
    url: buildUrl(req, '/apps/search') + '?' + qs.stringify({ q: term })
  });
...
router.get('/apps/list', function (req, res, next) {
  function paginate(apps) {
    ...
    if (start - num >= 0) {
      ...
      apps.prev = buildUrl(req, '/apps/list') + '?' + qs.stringify(req.query);
    }

    if (start + num <= 500) {
      req.query.start = start + num;
      apps.next = buildUrl(req, '/apps/list') + '?' + qs.stringify(req.query);
    }
...
router.get('/apps/:appId', function (req, res, next) {
  if (!req.params.appId) {
    return res.status(400).json({ message: 'appId parameter is required' });
  }

  const opts = { appId: req.params.appId, ...req.query };
...
router.get('/developers/:devId/', function (req, res, next) {
  if (!req.params.devId) {
    return res.status(400).json({ message: 'devId parameter is required' });
  }
